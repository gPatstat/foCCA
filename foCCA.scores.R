foCCA.scores <- function(fdobj1, cca.fit, centerfns=TRUE)
{
  library(fda)
  meanfun=cca.fit$fd.center
  
  canwtcoef1=cca.fit$ccawtfd1$coefs
  ...
  if (centerfns) {
    ...
    sub=as.numeric(fdobj1[d]$coefs-meanfun$coefs)
    fdobj1$coefs[,d]=sub
    }
  }
